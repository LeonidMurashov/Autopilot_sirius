...
from environment import Environment, BUSES_COUNT
...
test_model_path = 'models/model-{}/qlearning.ckpt-3118000'.format(MODEL_NUMBER)
# Atari game to learn
...
#   Utils Parameters
# =============================
PROJECT_DIR = "C:\\Users\\user\\Desktop\\autopilot\\Leonid\\"#"D:\\Python_projects\\Autopilot_sirius\\Leonid\\"
# Display or not gym evironment screens
show_training = False#True
# Directory for storing tensorboard summaries
summary_dir = PROJECT_DIR + "log"
summary_interval = 100
checkpoint_path = PROJECT_DIR + 'models/model-{}\qlearning.tflearn.ckpt'.format(MODEL_NUMBER)
...
def build_dqn(num_actions, action_repeat):
	...
	Building a DQN.
	"""
	inputs = tf.placeholder(tf.float32, [None, action_repeat, NUM_FEATURES])  # 60
...
class AtariEnvironment(object):
	...
	def step(self, action_index):
		...
		s_t1 = np.empty((self.action_repeat, BUSES_COUNT, NUM_FEATURES))
	...
	while T < TMAX:
		...
		while True:
			...
			for i in range(BUSES_COUNT):
				...
				readout_t = q_values.eval(session=session, feed_dict={s: [s_t[:,i]]})

				# Choose next action based on e-greedy policy
				a_t_sub = np.zeros([num_actions])
				if random.random() <= epsilon:
					action_index.append(random.randrange(num_actions))
				else:
					action_index.append(np.argmax(readout_t))
				a_t_sub[action_index] = 1
				a_t.append(a_t_sub)
			...
			a_batch.append(a_t[0])
			s_batch.append(s_t[:,0]) #TODO that is bad
			...
			if t % checkpoint_interval == 0:
				if not os.path.exists(PROJECT_DIR + "models/model-{}".format(MODEL_NUMBER)):
					os.mkdir(PROJECT_DIR + "models/model-{}".format(MODEL_NUMBER))
				saver.save(session, PROJECT_DIR + "models/model-{}/qlearning.ckpt".format(MODEL_NUMBER), global_step=t)
...
def get_num_actions():
	...
	env = Environment(show_training or testing)
...
def train(session, graph_ops, num_actions, saver):
	...
	envs = [Environment(show_training or testing) for i in range(n_threads)]
...
def evaluation(session, graph_ops, saver):
	...
	monitor_env = Environment(show_training or testing)
